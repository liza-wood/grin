...
dtdev <- dtdev[dtdev$year > 2000 & !is.na(dtdev$year),]
...
Sys.time() # takes 2 minutes per 25
seconds_per_accession <- 182/25
seconds_per_accession*nrow(dtdev)/60/60/24 # This is 144 hours... 6 days
for(i in 1:length(ids)){ # length(ids)
  ...
  if(i %in% seq(1000, 100000, by = 1000)){
    saveRDS(total_list, "~/OneDrive - University of Exeter/data/grin/grin_db_l_short.RDS")
